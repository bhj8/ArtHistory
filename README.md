...
包含时代与地域地图、图解词典、多作品图库、搜索筛选、主题路线、收藏及两项对照。全部 233 个条目已有配图，其中 176 个条目支持多图浏览。配图包括作品、建筑、展览现场、工艺记录和 4 幅明确标注的本站学习图解。支持只看有图、最近阅读、卡片多图预览和大图细节放大。条目在居中宽幅阅读页中打开，桌面并排展示图文，手机上下阅读；支持页内逐幅翻图。内容覆盖史前至当代及设计、视觉媒介；这是学习导航，并非穷尽的世界美术史。

## 本地开发

条目按学习顺序分为 42 个「核心必读」、79 个「重点了解」、112 个「专题拓展」。首页默认核心主线，可切换核心＋重点或全部条目，并按学习优先排序。42 个核心条目均包含历史背景、重点导读、自测与下一步阅读。层级保存在分享链接中；搜索会提示其他层级的匹配结果。
