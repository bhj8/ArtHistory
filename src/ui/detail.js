import { esc, link, imageHTML } from "./helpers.js";
export function detailHTML(d, c, { saved, compare, artIndex = 0 }) {
 ...
 ${a ? `<section class="detail-gallery" aria-label="作品图"><div class="gallery-heading"><h3>看作品，理解概念</h3><span>${works.length} 件作品 · 点图放大</span></div><button class="detail-art" data-light="${a.id}" aria-label="放大${esc(a.zh)}">${imageHTML(a, "", true)}<span aria-hidden="true">放大 ↗</span></button><div class="art-caption"><strong>${esc(a.zh)}</strong><span>${esc(a.artistZh || a.artist)} · ${esc(a.date)}</span>${a.medium ? `<span>${esc(a.medium)}</span>` : ""}<span>${esc(a.museum || "")}</span>${link(a.url, "馆藏记录")}<span class="art-position">${artIndex + 1} / ${works.length}</span></div><aside class="looking-guide"><strong>带着问题看</strong><p>${esc(d.look)}</p>${a.notes?.[d.id] ? `<p>${esc(a.notes[d.id])}</p>` : ""}</aside>${works.length > 1 ? `<div class="thumbnails" aria-label="切换作品">${works.map((w, i) => `<button data-thumb="${i}" class="${i === artIndex ? "on" : ""}" aria-label="作品 ${i + 1}：${esc(w.zh)}" aria-pressed="${i === artIndex}">${imageHTML(w)}<span>${esc(w.zh)}</span><small>${esc(w.date)}</small></button>`).join("")}</div>` : ""}</section>` : ""}
 ...
 <section class="detail-section"><h3>阅读资料</h3><div class="resource-links">${sourceIds
   ...
   .map((w) =>
     link(
       w.url,
       `<b>${esc(w.zh)}</b><small>${esc(w.museum || "The Met")} · 馆藏记录</small>`,
     ),
   )
   .join(
     "",
