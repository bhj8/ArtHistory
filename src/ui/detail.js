...
export function detailHTML(d, c, { saved, compare, artIndex = 0 }) {
 ...
 <div class="detail-reading">${d.study ? `<section class="study-guide"><span class="study-eyebrow">核心导读</span><h3>为什么先读</h3><p>${esc(d.study.why)}</p><h3>抓住这两点</h3><ul>${d.study.takeaways.map((t) => `<li>${esc(t)}</li>`).join("")}</ul></section>` : ""}${section("视觉特征", d.look)}${section("历史背景", d.context || d.idea)}${d.context ? section("核心问题", d.idea) : ""}${section("人物与作品", [d.people, d.work])}${d.reading ? section("观看要点", d.reading) : ""}<aside class="distinguish"><h3>辨别</h3><p>${esc(d.distinguish)}</p></aside>
 ${d.study ? `<section class="detail-section study-check"><h3>试着说清楚</h3><p>${esc(d.study.question)}</p><details><summary>展开参考思路</summary><p>${esc(d.study.answer)}</p></details></section><section class="detail-section"><h3>接着读</h3><div class="study-next">${d.study.next.map((id) => `<button data-node="${id}">${levelBadge(BYID[id])}<strong>${esc(BYID[id].zh)} →</strong><small>${esc(BYID[id].hook)}</small></button>`).join("")}</div></section>` : ""}
 ${
   related.length
