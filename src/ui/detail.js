...
export function detailHTML(d, c, { saved, compare, artIndex = 0 }) {
 ...
 ${a ? `<section class="detail-gallery" aria-label="作品图"><button class="detail-art" data-light="${a.id}" aria-label="放大${esc(a.zh)}">${imageHTML(a, "", true)}<span aria-hidden="true">放大 ↗</span></button><div class="art-caption"><strong>${esc(a.zh)}</strong><span>${esc(a.artistZh || a.artist)} · ${esc(a.date)}</span>${a.medium ? `<span>${esc(a.medium)}</span>` : ""}${link(a.url, "馆藏记录")}<span class="art-position">${artIndex + 1} / ${works.length}</span></div>${works.length > 1 ? `<div class="thumbnails" aria-label="切换作品">${works.map((w, i) => `<button data-thumb="${i}" class="${i === artIndex ? "on" : ""}" aria-label="作品 ${i + 1}：${esc(w.zh)}" aria-pressed="${i === artIndex}">${imageHTML(w)}</button>`).join("")}</div>` : ""}</section>` : ""}
   ...
   related.length
     ? `<section class="detail-section"><h3>相关条目 <span>${related.length}</span></h3><div class="related">${related
         .map((e) => {
           ...
           return `<button data-node="${target}"><b>${esc(BYID[target].zh)} <span>↗</span></b><small>${esc(e[2])} · ${esc(e[3])}</small></button>`;
         })
         .join("")}</div></section>`
     : ""
 }
