...
export function detailHTML(d, c, { saved, compare, artIndex = 0 }) {
  ...
  const section = (title, text) =>
    factualText(text)
      ? `<section class="detail-section"><h3>${title}</h3>${(Array.isArray(text) ? text : [text]).map(factualText).filter(Boolean).filter((p, i, all) => !all.some((other, j) => j < i && other.includes(p.replace(/[。.]$/, "")))).map((p) => `<p>${esc(p)}</p>`).join("")}</section>`
      : "";
 ...
 ${a ? `<section class="detail-gallery" aria-label="作品与学习图解"><div class="gallery-heading"><h3>作品</h3><span>${works.length} 幅配图 · 点图放大</span></div>${works.length > 1 ? `<div class="gallery-controls"><button data-thumb="${artIndex - 1}" ${artIndex === 0 ? "disabled" : ""} aria-label="上一幅配图">← 上一幅</button><span>${artIndex + 1} / ${works.length}</span><button data-thumb="${artIndex + 1}" ${artIndex === works.length - 1 ? "disabled" : ""} aria-label="下一幅配图">下一幅 →</button></div>` : ""}${works.length > 1 ? `<div class="thumbnails" aria-label="切换配图">${works.map((w, i) => `<button data-thumb="${i}" class="${i === artIndex ? "on" : ""}" aria-label="配图 ${i + 1}：${esc(w.zh)}" aria-pressed="${i === artIndex}">${imageHTML(w)}<span>${esc(w.zh)}</span><small>${esc(w.date)}</small></button>`).join("")}</div>` : ""}<button class="detail-art" data-light="${a.id}" aria-label="放大${esc(a.zh)}">${imageHTML(a, "", true)}<span aria-hidden="true">放大 ↗</span></button><div class="art-caption"><strong>${esc(a.zh)}</strong><span>${esc(a.artistZh || a.artist)} · ${esc(a.date)}</span>${a.medium ? `<span>${esc(a.medium)}</span>` : ""}<span>${esc(a.museum || "")}</span>${link(a.url, esc(a.sourceLabel || "馆藏记录"))}<span class="art-position">${artIndex + 1} / ${works.length}</span></div><details class="image-provenance"><summary>图片来源与署名</summary>${creditHTML(a)}</details>${factualText(a.notes?.[d.id]) ? `<details class="art-note"><summary>作品补充</summary><p>${esc(factualText(a.notes[d.id]))}</p></details>` : ""}</section>` : ""}
 <div class="detail-reading">${section("视觉特征", d.look)}${section("历史背景", d.context || d.idea)}${section("人物与作品", [d.people, d.work])}<aside class="distinguish"><h3>辨别</h3><p>${esc(factualText(d.distinguish))}</p></aside>
 ${d.study ? `<section class="detail-section"><h3>延伸条目</h3><div class="study-next">${d.study.next.map((id) => `<button data-node="${id}">${levelBadge(BYID[id])}<strong>${esc(BYID[id].zh)} →</strong><small>${esc(BYID[id].hook)}</small></button>`).join("")}</div></section>` : ""}
 ${
   related.length
